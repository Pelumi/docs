...
### March 2026
- **Checkout** `docs` – Introduced the optional `allowedPaymentMethods` field in the checkout order request, enabling merchants to restrict the payment methods shown on the checkout page (e.g., `Card`, `Transfer`). See update in [Checkout](/products/accept-payment/create-checkout-order).
