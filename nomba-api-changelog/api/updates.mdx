...
- **Global Payout** `added` – New endpoint `GET /v1/global-payout/mobile-money/providers` returns the list of mobile money providers available for DRC transfers (Mpesa, Airtel Money, Orange Money). Use the returned `code` and `displayName` as `institutionCode` and `institutionName` in the Authorize Transfer request. See [List Mobile Money Providers](/docs/products/global-payout/mobile-money-providers).
