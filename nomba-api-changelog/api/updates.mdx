...
### August 2026
- **Checkout** `docs` – Documented **Pay by Bank**, which lets customers with supported UK and European bank accounts authorise a payment directly in their banking app, with no card details entered at checkout. Use the `allowedPaymentMethods` value `Intl Transfer` to request or restrict it. It is available on supported `GBP` and `EUR` checkout flows, must be enabled on your account, and does not support recurring payments. See [Payment Methods](/docs/products/accept-payment/payment-methods#pay-by-bank).
