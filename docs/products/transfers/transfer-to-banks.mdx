...
<Tip>
  To test your 201 handling in sandbox, send a bank transfer to `accountNumber` `0000000201` (the webhook reports success) or `0000000202` (the webhook reports a refund). See [Transfer Test Scenarios](/docs/api-basics/testing/bank-transfer#transfer-test-scenarios).
</Tip>
...
Use `https://sandbox.nomba.com/v2/transfers/bank` with your sandbox credentials. Transfers succeed by default. To test other outcomes, set `accountNumber` to one of these values:

| `accountNumber` | Outcome |
|---|---|
| `0000000201` | `201` pending response, then a `payout_success` webhook about 5 seconds later |
| `0000000202` | `201` pending response, then a `payout_refund` webhook about 5 seconds later |
| `0000000451` | `400` insufficient balance, no webhook |
| `0000000409` | `400` duplicate `merchantTxRef`, no webhook |

See [Transfer Test Scenarios](/docs/api-basics/testing/bank-transfer#transfer-test-scenarios) for full request and response examples.
