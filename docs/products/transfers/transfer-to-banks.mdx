...
#### Requery Endpoints
- If you are transferring from a parent account, use [this](/nomba-api-reference/transactions/fetch-a-single-transaction-on-the-parent-account)
``` bash 
GET /v1/transactions/accounts/single?transactionRef=API-TRANSFER-XXX-XXX
 ```

- If you are transferring from a sub account, use [this](/nomba-api-reference/transactions/fetch-a-single-transaction-on-a-sub-account)
``` bash 
GET /v1/transactions/accounts/{subAccountId}/single?transactionRef=API-TRANSFER-XXX-XXX

 ```
- If you use both parent and sub account to carry out transfers, use [this](/nomba-api-reference/transactions/transaction-requery)
``` bash 
...
- You can safely retry the transaction using after a refund using a new reference.
...
- Do NOT generate a new reference for the same transaction
- If a transaction returns `PENDING` do NOT re-initiate the transfer with a different reference, the transaction may still be processed successfully.
- Use the Transaction requery endpoints discussed above to confirm the final status, do not rely solely on the initial response
...
- When in doubt, treat unknown responses as `PENDING`
