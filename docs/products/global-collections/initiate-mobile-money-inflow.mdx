...
<ParamField body="phoneNumber" type="string" required>
  The customer's mobile money phone number in international format (e.g., `+243XXXXXXXXX`).
</ParamField>
...
<ParamField body="currency" type="string" required>
  The ISO 4217 currency code for the collection (e.g., `CDF`, `KES`, `UGX`).
</ParamField>

<ParamField body="topupVendor" type="string" required>
  The mobile money network provider to use (e.g., `AIRTEL`, `MPESA`).
</ParamField>

#### Response body

<ResponseField name="transactionReference" type="string" required>
  The unique reference for this collection. Use this with the FetchMobileMoneyTransaction endpoint to check the status.
</ResponseField>
