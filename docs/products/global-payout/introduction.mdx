...
<Note>
  Convert Money is optional. Use it to calculate the destination amount for a source amount. To use a locked rate for a cross-currency transfer, pass the `exchangeRateId` returned by Fetch Exchange Rates as `lockedExchangeRateId` in the Authorize Transfer request.
</Note>
