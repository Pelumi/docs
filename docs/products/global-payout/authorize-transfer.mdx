...
<Tip>
  For DRC transfers, call [List Institution Providers](/docs/products/global-payout/mobile-money-providers) first to get valid `institutionCode` and `institutionName` values. Use `isMobileMoney=true` for mobile money providers and `isMobileMoney=false` for banks.
</Tip>
