...
<Note>
  To have a cross-currency transfer use the rate you quoted to your customer, pass the `exchangeRateId` in the response as `lockedExchangeRateId` in your [Authorize Transfer](/docs/products/global-payout/authorize-transfer) request.
</Note>
...
<ResponseField name="rates" type="array" required>
  ...
  <Expandable title="rate object" defaultOpen="true">
    <ResponseField name="exchangeRateId" type="string">
      The unique ID of this exchange rate. Pass it as `lockedExchangeRateId` in Authorize Transfer for the transfer to use this rate.
    </ResponseField>
