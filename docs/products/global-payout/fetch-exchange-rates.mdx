...
<Note>
  To lock the quoted rate for a cross-currency transfer, pass the `exchangeRateId` in the response as `lockedExchangeRateId` in your [Authorize Transfer](/docs/products/global-payout/authorize-transfer) request.
</Note>
...
<ResponseField name="rates" type="array" required>
  ...
  <Expandable title="rate object" defaultOpen="true">
    <ResponseField name="exchangeRateId" type="string">
      The unique ID of this exchange rate. Pass it as `lockedExchangeRateId` in Authorize Transfer to use this rate for the transfer.
    </ResponseField>
