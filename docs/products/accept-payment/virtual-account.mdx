...
<Note>
  ...
  - Sandbox virtual accounts can only receive ₦100 per transfer, and funds can be transferred from any Nigerian bank.
    All created account have also been updated from ₦10 old value to ₦100 
 
  - Merchants can update the expected amount from ₦100 to any value not exceeding ₦150.  
    Set the `expectedAmount` field when creating the virtual account to apply this change.
...
- `expectedAmount` (optional): Restricts the account to accept only the specified amount. This is useful for exact payments such as invoice matching or controlled transfers.  When omitted, the account will accept any amount since it is not restricted to a specific value. 
  ...
  If a sender attempts to transfer an amount different from the expected value, the transaction may be declined by the sender’s bank or automatically reversed, depending on the bank’s handling of such transfers.
