...
<Note>
  - Each user can create a maximum of **2 virtual accounts** you can expire old accounts to create more.
  - Each account is created with an `expectedAmount` of ₦100 and accepts only that exact amount
...
- `nin` (optional): The account holder's National Identification Number, exactly 11 digits. You can send it on its own instead of a `bvn`, or alongside `bvn` to record both. If you send neither, the virtual account inherits the BVN of the parent account as described above.
...
### Test the account in the Sandbox

To confirm the account works, send ₦100 to the `bankAccountNumber` returned above, from any Nigerian bank.

To test expected amount usecase, update the account with an `expectedAmount` between ₦100 and ₦150, then transfer exactly that amount.

<Card
  title="Testing virtual accounts"
  icon="flask"
  href="/docs/api-basics/testing/virtual-accounts"
>
  Full sandbox walkthrough — create an account, simulate an inflow, and test the expected amount.
</Card>

## Expire a virtual account

Expiring an account stops it accepting further payments. Use it when a customer's account is no longer needed. For example, a dynamic account for a one-off order.

To expire a virtual account, send a [DELETE request](/nomba-api-reference/virtual-accounts/expire-a-virtual-account) to `/v1/accounts/virtual/{identifier}`, where `identifier` is the `accountRef` you set when creating the account or the `bankAccountNumber` that was returned.
