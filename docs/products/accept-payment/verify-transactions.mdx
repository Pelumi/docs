...
|---|---|---|---|
| `/v1/transactions/accounts/single` | GET | Sandbox + Production | You want to confirm `status: SUCCESS` before delivering value. Works with `orderReference`, `orderId`, `transactionRef`, or `merchantTxRef` as query params. |
...
This endpoint works in both sandbox and production. Pass the `orderReference` (the reference you supplied on the order), the `orderId` (the value returned as `orderReference` in the create-order response), or the `transactionRef` (the Nomba transaction ID from the webhook) as a query parameter.
